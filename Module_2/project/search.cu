...
__global__ void search(int *d_d, int *d_i, int numElements)
{
    ...
    if (idx < numElements && d_d[idx] == d_v) {
        // Use atomicCAS to safely set only if still -1 (first match wins)
        int expected = -1;
        while (expected == -1) {
            if (atomicCAS(d_i, expected, idx) == expected) {
                break;
            }
            expected = *d_i;
            if (expected != -1) break;
...
__host__ std::tuple<int *, int> readCsv(std::string filename)
{
    ...
    // ✅ FIX: Allocate on heap so pointer remains valid after return
...
__host__ std::tuple<int *, int *> allocateDeviceMemory(int numElements)
{
    ...
    cudaMemset(d_i, -1, sizeof(int)); // 👈 ADD THIS LINE
...
__host__ std::tuple<int *, int, int> setUpSearchInput(std::string inputFilename, int numElements, int h_v, bool sortInputData)
{
    ...
    {
        auto csvData = readCsv(inputFilename);
        h_d = get<0>(csvData);
...
int main(int argc, char *argv[])
{
    ...
    auto searchInputTuple = setUpSearchInput(inputFilename, numElements, h_v, sortInputData);
    ...
    cudaDeviceSynchronize(); // 👈 ADD THIS LINE HERE
